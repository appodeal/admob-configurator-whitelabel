var ReportingStepFourController;
...
ReportingStepFourController = (function () {
    ...
    var initOtherLibrary, waitForCredentials, outhclientPageLink, startCredentialsCreating,
        ...
        findAppodealClient, modal, credentials_interval, redirect_uri = REDIRECT_URI,
        waitUntilClientInfoPresent, addCredentials, wl_projectName, modal_header, url_admediator = 'www.appodeal.com';
    ...
    checkAndSaveClientCredentials = function (clientId, clientSecret) {
        ...
        if (clientId && clientSecret) {
            ...
            console.log('Go to the ' + wl_projectName + ' web client');
    ...
    addAdmobAccount = function (clientId, clientSecret, account_id, appodeal_api_key, appodeal_user_id) {
        chrome.storage.local.get({
            ...
            try {
                url = 'https://' + url_admediator + '/api/v1/add_admob_account.json';
                ...
                modal.show(modal_header, 'Please grant permission to ' + wl_projectName + ' to read your Admob reports.<br>You will be automatically redirected in 5 seconds.');
            } catch (err) {
                airbrake.error.notify(err);
            }
            setTimeout((function () {
                    ...
                    setInterval((function () {
                        ...
                        try {
                            if (http.readyState === 4 && http.status === 200) {
                                message = 'Admob account created on ' + wl_projectName + '.';
                                console.log(message);
                                response = JSON.parse(http.responseText);
                                if (response['id'] === null) {
                                    console.log('Error creating admob account on ' + wl_projectName + '. Field id not null');
                                    return;
                                }
                                local_settings = {
                                ...
                                message = 'Error creating admob account on ' + wl_projectName;
                                sendOut(1, message);
                                modal.show(modal_header, message);
                                chrome.storage.local.remove('reporting_tab_id');
                                throw new Error(message);
        ...
        try {
            ...
            $.each(download_links, function () {
                ...
                if (data.web && data.web.javascript_origins) {
                    ...
                    if (names.includes('http://' + url_admediator + '/') || names.includes('http://' + url_admediator) || names.includes('https://' + url_admediator) || names.includes('https://' + url_admediator + '/')) {
    ...
    findAppodealClient = function () {
        try {
            return jQuery("tr[pan-table-row] td a[content*='" + url_admediator + "/admin/oauth2callback']").parents('tr[pan-table-row]');
        } catch (err) {
            airbrake.error.notify(err);
        }
    };
    ...
    addCredentials = function () {
        ...
        try {
            origins = "['http://" + url_admediator + "/', 'https://" + url_admediator + "/']";
            redirectUris = "['http://" + url_admediator + "/admin/oauth2callback', 'https://" + url_admediator + "/admin/oauth2callback']";
            ...
            setTimeout((function () {
                ...
                setTimeout((function () {
                    ...
                    name_code = 'angular.element(jQuery("' + ':input[ng-model=\'oAuthEditorCtrl.client.displayName\']")).controller().client.displayName = \'' + wl_projectName + ' client\';';
    ...
    return {
        init: function () {
            initOtherLibrary('Create and sync credentials.', function () {
...
// var url_admediator = 'www.appodeal.com';
// var redirect_uri = "https://"+url_admediator+"/admin/oauth2callback";
//
// jQuery(function () {
...
//         var origins = "['http://"+url_admediator+"/', 'https://"+url_admediator+"/']";
//         var redirectUris = "['http://"+url_admediator+"/admin/oauth2callback', 'https://"+url_admediator+"/admin/oauth2callback']";
//         console.log("Redirected to oauthclient creating page.");
//
//         setTimeout(function () {
...
//             setTimeout(function () {
//                 console.log("Insert display name, redirect and origins urls");
//                 name_code = "angular.element(jQuery(\"" + ":input[ng-model='oAuthEditorCtrl.client.displayName']\")).controller().client.displayName = '"+projectName+" client';";
//                 origins_code = "angular.element(jQuery(\"ng-form[ng-model='oAuthEditorCtrl.client.postMessageOrigins']\")).controller().client.postMessageOrigins = " + origins + ";";
//                 redirect_uris_code = "angular.element(jQuery(\"ng-form[ng-model='oAuthEditorCtrl.client.redirectUris']\")).controller().client.redirectUris = " + redirectUris + ";";
//                 submit_form_code = "angular.element(jQuery(\"form[name='clientForm']\")).controller().submitForm();";
//                 run_script(name_code + origins_code + redirect_uris_code + submit_form_code);
...
//         var tr = jQuery("tr[pan-table-row] td a[content*='"+url_admediator+"/admin/oauth2callback']").parents('tr[pan-table-row]');
//         return tr;
//     }
...
//             };
//             console.log(JSON.stringify(json));
//             modal.show(projectName + " Chrome Extension", "Please grant permission to " + projectName + " to read your Admob reports.<br>You will be automatically redirected in 5 seconds.");
//             setTimeout(function () {
...
//                         debugger;
//                         if (http.readyState == 4 && http.status == 200) {
//                             message = 'Admob account created on ' + projectName + '.';
//                             console.log(message);
//                             var response = JSON.parse(http.responseText);
//                             if(response['id']==null){
//                                 console.log("Error creating admob account on "+projectName.toLowerCase()+". Field id not null");
//                                 return
//                             }
//                             var local_settings = {
//                                 reporting_client_creating: true,
...
//                             })
//                         } else {
//                             message = "Error creating admob account on " + projectName.toLowerCase();
//                             sendOut(1, message);
//                             modal.show(projectName+" Chrome Extension", message);
//                             chrome.storage.local.remove("reporting_tab_id");
//                         }
...
//         } else if (clientId) {
//             console.log("Credential client_id found, but client_secret not found. Try to reset.");
//             console.log("Go to the "+ projectName + " web client");
//
...
//                 appendJQuery(function () {
//                     modal = new Modal();
//                     modal.show(projectName + " Chrome Extension", "Preparing credentials.");
//                     if (isOauthClientPage()) {
//                         console.log("Oauth client page");
//                         addCredentials();
//                     } else if (isCredentialClientPage()) {
...
//                 appendJQuery(function () {
//                     modal = new Modal();
//                     modal.show("AdMediator Chrome Extension", "Please enter to url your AdMediator");
//                 });
//                 return false;
//             }
//         });
