var Admob = function (userId, apiKey, publisherId, accountEmail, accounts, interstitialBids, bannerBids, mrecBids, rewarded_videoBids, wl_projectName, modal_header) {
...
Admob.prototype.finishDialog = function () {
    ...
    try {
        ...
        items.push("<h4>Admob is synced with " + self.wl_projectName + " now.</h4>");
        self.modal.show("Good job!", "Admob is synced with " + self.wl_projectName + " now. You can run step 3 again if you add new apps.<h3>Synchronized inventory</h3>" + self.humanReport().join(""));
        // send finish reports
        self.sendReports({
            mode: 0,
            timeShift: 1000
...
Admob.prototype.showErrorDialog = function (content) {
    var self = this;
    var message = "Sorry, something went wrong. Please restart your browser and try again or contact " + self.wl_projectName + " support.<h4>" + content + "</h4>";
...
Admob.prototype.defaultAppName = function (app) {
    var self = this, maxLength = 80, name;
    name = self.wl_projectName +'/' + app.id + "/" + app.appName;
...
Admob.prototype.adUnitRegex = function (name) {
    var result = {}, self = this, matchedType;
...
Admob.prototype.mapApps = function (callback) {
    ...
    var self = this, regex, appodealMatch;
