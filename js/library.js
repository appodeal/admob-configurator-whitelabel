...
LibraryController = function () {
    return {
        ...
        projectidsuggestion: function (callback) {
            var random = projectName.toLowerCase() + '-' + LibraryController.random_string(10);
            $.ajax
            ({
                type: "GET",
        ...
        create: function () {
            modal.show(projectName + " Chrome Extension", "Create " + projectName + " project. Please wait");
                            ...
                            $.each(data.items, function (index, value) {
                                if (value.descriptionLocalizationArgs.assignedIdForDisplay === id) {
                                    ...
                                    if (value.status === "DONE") {
                                        ...
                                        message = "Sorry, something went wrong. Please restart your browser and try again or contact " + projectName + " support. </br> <h4>" + value.error.causeErrorMessage + "</h4>";
        ...
        chromeStorageGet(function (updates) {
            if (updates.wl_host) {
                appendJQuery(function () {
                    modal = new Modal();
                    modal.show(projectName + " Chrome Extension", "Find " + projectName + " project. Please wait");
                });
                console.log("Find " + projectName + " project. Please wait");
