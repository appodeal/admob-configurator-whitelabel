...
LibraryController = function () {
    ...
    projectIdSuggestion = function (callback) {
        var random = wl_projectName.toLowerCase()+ '-' + random_string(10, 'N');
        $.ajax({
            type: "GET",
                        ...
                        if (response.readyState === 4 && response.status === 200) {
                            ...
                            if (data) {
                                ...
                                data.default.resource.forEach(function (value, index, arr) {
                                    if (value.display_name === wl_projectName) {
    ...
    create = function () {
        modal.show(modal_header, 'Create ' + wl_projectName + ' project. Please wait');
        try {
                                ...
                                if (value.descriptionLocalizationArgs.assignedIdForDisplay === id_project) {
                                    ...
                                    switch (value.status) {
                                        ...
                                        case 'FAILED':
                                            ...
                                            message = "Sorry, something went wrong. Please restart your browser and try again or contact " + wl_projectName + " support. </br> <h4>" + value.error.causeErrorMessage + "</h4>";
                                            modal.show(modal_header, message);
                                            clearInterval(refreshIntervalId);
    ...
    return {
        init: function () {
            initOtherLibrary('Create new project from the library page (new accounts)', function () {
